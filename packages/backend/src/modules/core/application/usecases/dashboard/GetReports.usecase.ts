...
export class GetVisitsUseCase {
    ...
    async execute(
        ...
        const engineer = await this.engineerRepository.getReports(
            engineerId,
            clientId,
            visitId,
        );
        ...
        if (!engineer.value.clients[0].visits[0].reports.length) {
            return Res.failure(new RepositoryNoDataFound('No reports found'));
        }
